...
class _SettingsState extends State<Settings> {
  ...
  readSettings() async {
    ...
    int workTime = prefs.getInt(WORKTIME) ?? 1;
    if (workTime == null) {
      await prefs.setInt(WORKTIME, int.parse('30'));
    }
    int shortBreak = prefs.getInt(SHORTBREAK) ?? 1;
    if (shortBreak == null) {
      await prefs.setInt(SHORTBREAK, int.parse('5'));
    }
    int longBreak = prefs.getInt(LONGBREAK) ?? 1;
    if (longBreak == null) {
  ...
  void updateSetting(String key, int value) {
    switch (key) {
      ...
      case SHORTBREAK:
        {
          int short = prefs.getInt(SHORTBREAK) ?? 1;
          short += value;
      ...
      case LONGBREAK:
        {
          int long = prefs.getInt(LONGBREAK) ?? 1;
          long += value;
          if (long >= 1 && long <= 180) {
